...
time_series_split <- function(data, n_splits) {
  ...
  indices <- 1:n_samples  # Maintain chronological order of indices
  split_points <- seq(size_of_each_split, by = size_of_each_split, length.out = n_splits)
...
for (attempt in 1:max_attempts) {
  ...
  # Display model and R-squared value
  cat("Attempt:", attempt, "\n")
  ...
  # Check if R-squared exceeds threshold or maximum attempts reached
  if (best_r_squared >= 0.7 || attempt == max_attempts) {
    break  # Exit loop
  }
}

# Output best hyperparameters and performance
cat("Final Best R-squared:", best_r_squared, "\n")
cat("Final Best Model:", "\n")
print(best_model)
