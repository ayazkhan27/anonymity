...
train_data <- train_data[ , !names(train_data) %in% c("Title")]

test_data <- test_data[ , !names(test_data) %in% c("Title")]

cols_with_na <- sapply(train_data, function(x) any(is.na(x)))
...
sentiment_formula_3 <- mean_sentiment ~ scaled_buffer_ratio + scaled_neutral_ratio +
  log_emotion_ratio + interaction_term + scaled_positive_ratio + scaled_negative_ratio + decay_factor

# Train the random forest model using the defined formula
sentiment_model_3 <- randomForest(
  formula = sentiment_formula_3,
  data = train_data,
  ntree = 80,
...
train_data$predicted_sentiment <- predict(sentiment_model_3, newdata = train_data)
test_data$predicted_sentiment <- predict(sentiment_model_3, newdata = test_data)
...
predicted_sentiment <- predict(sentiment_model_3, newdata = test_data)
...
model <- train(sentiment_formula_3, data = combined_data,
...
yearly_sentiments <- combined_data %>%
  group_by(year) %>%
  summarise(mean_actual_sentiment = mean(mean_sentiment, na.rm = TRUE),
...
varImpPlot(sentiment_model_3)

# Get variable importance
var_importance <- importance(sentiment_model_3)
...
library(randomForest)
plot(sentiment_model_3)

# Save the model to an RDS file
saveRDS(sentiment_model_3, "sentiment_model_3.rds")

# Optionally, you can also save it using `save`, which saves an R object under its name in an .RData file
save(sentiment_model_3, file = "sentiment_model_3.RData")
