...
data_processed <- english_posts_data %>%
  # First, calculate total_words for each individual Post Content entry
  mutate(
    total_words = str_count(`Post Content`, "\\S+")
  ) %>%
  # Then, group by necessary identifiers to summarize other metrics
  group_by(Keyword, Page, `Post Number`, Title, `Post Content`, year, month, day_of_week, time, post_date) %>%
  summarise(
    total_words = first(total_words),  # Use first since total_words should be the same for grouped entries
...
# Calculate the mean_sentiment column
data_processed <- data_processed %>%
  mutate(mean_sentiment = ifelse(total_emotion_words > 0, total_sentiment / total_emotion_words, 0))
...
# Assuming the data_processed and steps prior have been completed correctly
# Splitting data into training and testing sets based on the date
...
rf_model <- randomForest(
  mean_sentiment ~ year + month + day_of_week + extreme_positive_ratio + extreme_negative_ratio +
    moderate_positive_ratio + moderate_negative_ratio + medium_positive_ratio + medium_negative_ratio +
    slight_positive_ratio + slight_negative_ratio + neutral_ratio + buffer_ratio,
  data = train_data,
  ntree = 500,
  mtry = 11  # Adjust this parameter based on the number of predictors and performance
)

# Check model importance and performance
...
# Add predictions to the test_data, and filter out the outlier predictions based on earlier defined bounds
test_data <- test_data %>%
  mutate(predicted_sentiment = predictions) %>%
  filter(predicted_sentiment >= lower_bound & predicted_sentiment <= upper_bound)

# Evaluate the model using caret's postResample function
...
# Plot the graph with data points to visualize if the data is plotting correctly
