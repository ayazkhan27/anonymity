...
extract_date <- function(content) {
  date_pattern <- "(\\b\\d{1,2}[/-]\\d{1,2}[/-]\\d{2,4}\\b)|(\\b\\w+ \\d{1,2}(st|nd|rd|th)? \\w+ \\d{4}\\b)|(\\b\\w+ \\d{1,2}(st|nd|rd|th)?,? \\d{4}\\b)|(\\b\\w+ \\d{4}\\b)|(\\b\\d{1,2}/\\d{1,2}/\\d{2,4}\\b)"
  date_match <- regmatches(content, regexpr(date_pattern, content, ignore.case = TRUE))
  date <- as.Date(date_match, format = c("%m/%d/%y", "%m/%d/%Y", "%d-%m-%y", "%d-%m-%Y", "%d/%m/%y", "%d/%m/%Y", "%B %d, %Y", "%d %B %Y", "%Y-%m-%d", "%Y/%m/%d"))
  return(date)
}

# Function to extract year from Post Content
extract_year <- function(content) {
  year_pattern <- "(\\b\\d{4}\\b)"
  year_match <- regmatches(content, regexpr(year_pattern, content, ignore.case = TRUE))
...
# Assign extracted date directly to posts_data$post_date
posts_data$post_date <- extract_date(posts_data$`Post Content`)

# Remove rows with missing or NA values in the year column
posts_data <- posts_data[complete.cases(posts_data$post_date), ]
...
# Data Preparation
posts_data$year <- extract_year(posts_data$`Post Content`)
posts_data$month <- month(posts_data$post_date, label = TRUE)
posts_data$day_of_week <- wday(posts_data$post_date, label = TRUE)
